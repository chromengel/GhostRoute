...
private fun altTest(graphDir: String) {
    ...
    val toLat = 35.7973; val toLon = -84.2561
...
private fun turnTest(graphDir: String) {
    ...
    val fromLat = 35.7973; val fromLon = -84.2561
    val toLat = 35.8840; val toLon = -84.1558
...
private fun importGraph(pbf: String, graphDir: String) {
    ...
    val request = GHRequest(35.7973, -84.2561, 36.0104, -84.2696).setProfile("car")
...
private fun snapTest(graphDir: String) {
    ...
    val fromLat = 35.9550; val fromLon = -83.9400
    val toLat = 35.9620; val toLon = -83.9550
    val cams = listOf(Cam("node/0", 35.9580, -83.9470, 305.0))
    ...
    for (e in lastPathEdges ?: emptyList()) {
        ...
        for (i in 0 until g.size() - 1) cd = minOf(cd, pointToSegMeters(35.9580, -83.9470, g.getLat(i), g.getLon(i), g.getLat(i + 1), g.getLon(i + 1)))
        if (cd > 90.0) continue
        val eid = e.edge
    ...
    run {
        val w = CameraAvoidanceWeightingV2(base, fwd, rev, baseEdges, 100_000.0)
        val sf = DefaultSnapFilter(base, hopper.encodingManager.getBooleanEncodedValue(Subnetwork.key("car")))
        val s1 = hopper.locationIndex.findClosest(35.7973, -84.2561, sf)
        val s2 = hopper.locationIndex.findClosest(36.0104, -84.2696, sf)
        val qg = QueryGraph.create(hopper.baseGraph, listOf(s1, s2))
        val a = com.graphhopper.routing.AStar(qg, w, TraversalMode.NODE_BASED)
        lms?.let { a.setApproximation(LMApproximator.forLandmarks(qg, base, it, 8)) }
        val t0 = System.nanoTime()
    ...
    val omni = listOf(Cam("omni", 35.9580, -83.9470, null))
    ...
    for (i in 0 until fast.size() - 1) {
        val d = pointToSegMeters(35.9580, -83.9470, fast.getLat(i), fast.getLon(i), fast.getLat(i + 1), fast.getLon(i + 1))
    ...
    for (e in edges) {
        ...
        for (i in 0 until g.size() - 1) {
            val d = pointToSegMeters(35.9580, -83.9470, g.getLat(i), g.getLon(i), g.getLat(i + 1), g.getLon(i + 1))
...
private fun cameraTest(graphDir: String) {
    ...
    val fromLat = 35.7973; val fromLon = -84.2561 // TN test origin
    val toLat = 36.0104; val toLon = -84.2696      // TN test destination
